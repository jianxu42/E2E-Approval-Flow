...
def test_approval_teams(context: BrowserContext):
    ...
    approval_tab_view.get_by_role("menuitem", name="Dynamics FTE GCR (default)").click(timeout=0)
    approval_tab_view.get_by_role("button", name="Got it").click(timeout=0)
    approval_tab_view.get_by_role("menuitem", name="Dynamics FTE GCR (default)").click(timeout=0)
    approval_tab_view.get_by_role("menuitemcheckbox", name="JianTestSolution").click(timeout=0)
    approval_tab_view.get_by_role("gridcell", name=APPROVAL_FLOW_TITLE_FOR_TEAMS).click(timeout=0)
    approval_tab_view.get_by_role("button", name="Approve").click(timeout=0)
    page.get_by_role("button", name="Approvals Toolbar").click(timeout=0)
